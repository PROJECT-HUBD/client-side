...
// <-----------------------------***步驟:1.render畫面  2.算錢 3.Ajax 4.localstorage(不然畫面會出不來)--------------------------------->
  // <-----------------------------render畫面--------------------------------->
 ...
 $(document).off("click", ".buttonPlus").on("click", ".buttonPlus", function () {
    let countElement = $(this).closest("article").find(".quantity");
    let count = parseInt(countElement.text()); // 獲取當前數量
    countElement.text(count + 1); // 增加數量
    updatePrices(); // 更新所有價格
});

$(document).off("click", ".buttonMinus").on("click", ".buttonMinus", function () {
    let countElement = $(this).closest("article").find(".quantity");
    let count = parseInt(countElement.text()); // 獲取當前數量
    ...
    if (count > 1) {
        ...
        if (confirm("請問你要刪除商品嗎?")) {
            $(this).closest("article").remove(); // 刪除商品
        } else {
